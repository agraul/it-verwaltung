...
export class DeviceOverviewComponent implements OnInit {
  ...
  mapToDCards(devices: Device[]): Card[] {
    ...
    for(let i=0; i<devices.length; i++) {
      let card = new Card(devices[i].bezeichnung, devices[i].hersteller, false, devices[i].komponentenAttribute.find(attr => attr.id === this.getSeriennummerId()).value);
      cards[i] = card;
    }
    return cards;
  }
  ...
  getSeriennummerId(): number {
    return this.getDeviceAttributes().find(attr => attr.bezeichnung === "Seriennummer").id;
  }
